---
title: Prerequisites
description: How to configure your development environment for IAMPlatform.
type: guide
summary: Tools and accounts required before setting up IAMPlatform.
---

## Operating System

IAMPlatform is designed to work on macOS, Linux and Windows. While IAMPlatform itself is platform-agnostic, the tooling and dependencies we use have different requirements.

We've tested and confirmed that IAMPlatform works on the following combinations of operating systems, Node.js versions and IAMPlatform versions:

| Operating system | IAMPlatform version | Node.js version | Notes |
| ---------------- | ----------------- | --------------- | ----- |
...
We're aware of issues on [non-Ubuntu Linux distributions](https://github.com/vercel/IAMPlatform/issues/238). While we don't officially support them, we'd love to know if you get it working!

## Package Manager

IAMPlatform defaults to using [Bun](https://bun.sh) as a package manager, but you can use [npm](https://www.npmjs.com/), [yarn](https://yarnpkg.com/) or [pnpm](https://pnpm.io/) instead by passing a flag during the [installation](/docs/setup/installation) step.
...
IAMPlatform relies on various SaaS products. You will need to create accounts with the following services then set the API keys in your [environment variables](/docs/setup/env):
