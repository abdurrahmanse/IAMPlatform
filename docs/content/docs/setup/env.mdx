...
description: How to configure environment variables in IAMPlatform.
type: reference
...
IAMPlatform uses environment variables for configuration. This guide will help you set up the required variables to get started quickly, and optionally configure additional features.

All integration environment variables are **optional** — IAMPlatform will gracefully skip any integration that isn't configured. The only truly required variables are for core infrastructure (database and URLs).

## Quick Start (Minimum Setup)

To get IAMPlatform running locally with basic functionality, you only need to configure these **required** variables:
...
<Step>
  Fork the [IAMPlatform template](https://basehub.com/basehub/IAMPlatform?fork=1) on BaseHub
</Step>
...
IAMPlatform uses environment variables across multiple locations:
